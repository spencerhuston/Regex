...
void Regex::change_edges(std::shared_ptr<Fragment> & f1, std::shared_ptr<Fragment> & f2)
{
	if (f1->_edges->size() == 0)
	{
		std::shared_ptr<Edge> e(new Edge('\0', true, f1->_end, NULL));
		e->_out = f2->_start;
		e->_modifiable = true;
...
void Regex::change_edges(Fragment & f1, Fragment & f2)
{
	if (f1._edges->size() == 0)
	{
		std::shared_ptr<Edge> e(new Edge('\0', true, f1._end, f2._start));
		e->_modifiable = true;
		f1._edges->push_back(e);
	}
	else
...
void Regex::star(std::stack< std::shared_ptr<Fragment> > & nfa)
{
	...
	std::shared_ptr<Edge> e(new Edge('\0', true, f1->_start, NULL));
	e->_modifiable = true;
...
void Regex::plus(std::stack< std::shared_ptr<Fragment> > & nfa)
{
	...
	std::shared_ptr<Edge> e(new Edge('\0', true, f2._start, NULL));
	e->_modifiable = true;
...
void Regex::question(std::stack< std::shared_ptr<Fragment> > & nfa)
{
	...
	std::shared_ptr<Edge> e1(new Edge('\0', true, s, f1->_start));
	e1->_modifiable = true;
	std::shared_ptr<Edge> e2(new Edge('\0', true, s, f2->_start));
	e2->_modifiable = true;
	...
	std::shared_ptr<Edge> f2_e(new Edge('\0', true, f2_start, s2));	
